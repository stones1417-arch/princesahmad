(() => {
  ...
  function incidentMarkup(item) {
    const maintenance = item.maintenance ? `<section class="engineering-followup__maintenance"><h6>طلب الصيانة ${escapeHtml(item.maintenance.number)}</h6><p><strong>الحالة:</strong> ${escapeHtml(item.maintenance.status_label)}</p>${item.maintenance.technician ? `<small>الفني: ${escapeHtml(item.maintenance.technician)}</small>` : ""}${item.maintenance.planned_start ? `<small>الموعد المخطط: ${escapeHtml(item.maintenance.planned_start)} — ${escapeHtml(item.maintenance.planned_end)}</small>` : ""}${item.stage === "awaiting_close" ? '<div class="engineering-followup__maintenance-complete"><strong>اكتملت أعمال الصيانة</strong><span>ينتظر البلاغ تحقق مشرف البلاغات والإغلاق النهائي.</span></div>' : ""}</section>` : '<section class="engineering-followup__maintenance engineering-followup__maintenance--none">لا يوجد طلب صيانة مرتبط</section>';
    const update = item.last_update ? `<section class="engineering-followup__update"><strong>آخر تحديث من مشرف البلاغات</strong><p>${escapeHtml(item.last_update.note)}</p><small>${escapeHtml(item.last_update.actor)} · ${escapeHtml(item.last_update.created_at)}</small></section>` : "";
    const escalation = item.escalation !== "غير مصعّد" ? `<section class="engineering-followup__escalation"><strong>التصعيد: ${escapeHtml(item.escalation)}</strong>${item.escalation_note ? `<p>${escapeHtml(item.escalation_note)}</p>` : ""}<small>${escapeHtml(item.escalated_by)} ${escapeHtml(item.escalated_at)}</small></section>` : '<section class="engineering-followup__escalation engineering-followup__escalation--none"><strong>التصعيد:</strong> لا يوجد تصعيد</section>';
    ...
    return `<article class="engineering-followup" data-followup-incident="${item.id}" data-followup-filter="${item.is_closed ? "closed" : "open"} ${item.status} ${item.priority} ${item.escalation !== "غير مصعّد" ? "escalated" : ""} ${item.maintenance ? "maintenance" : "no-maintenance"}"><header><div><small>الباب المرتبط · ${escapeHtml(item.number)}</small><h5>${escapeHtml(item.number)}</h5><p>${escapeHtml(item.type)}</p></div><span class="stage-${item.stage}">${escapeHtml(item.stage_label)}</span></header><dl><div><dt>الأولوية</dt><dd>${escapeHtml(item.priority_label)}</dd></div><div><dt>وقت الإنشاء</dt><dd>${escapeHtml(item.created_at)}</dd></div><div><dt>مدة الانتظار / المعالجة</dt><dd>${waitingLabel(item.waiting_seconds)}</dd></div><div><dt>الحالة</dt><dd>${escapeHtml(item.status_label)}</dd></div></dl><section class="engineering-followup__responsibility"><h6>المسؤولية الحالية</h6><strong>${escapeHtml(item.assignee)}</strong><span>مشرف البلاغات</span></section>${escalation}${update}${maintenance}${item.closed_at ? `<p class="engineering-followup__closed">✓ تم إغلاق البلاغ · ${escapeHtml(item.closed_by)} · ${escapeHtml(item.closed_at)}</p>` : ""}<div class="engineering-followup__timeline-preview"><strong>آخر حدث</strong><span>${lastEvent ? `${escapeHtml(lastEvent.label)} — ${escapeHtml(lastEvent.created_at)}` : "لا توجد أحداث مسار مسجلة"}</span></div><details><summary>عرض المسار الكامل</summary><ol class="engineering-followup__timeline">${events || "<li><div><strong>لا توجد أحداث مسار مسجلة.</strong></div></li>"}</ol></details></article>`;
  }

  function renderFollowup(drawer, payload) {
    const body = drawer.querySelector("[data-followup-body]");
    const createAction = root.dataset.canCreateIncident === "true" ? `<a href="/ops/incidents/?engineering_door=${encodeURIComponent(payload.door.id)}&create=1" data-fixed-create-url="${escapeHtml(payload.create_url)}">إنشاء بلاغ تشغيلي</a>` : "";
    const supervisor = payload.supervisor ? `<section class="engineering-followup__leader"><div><small>مسؤول البلاغات للوردية</small><h5>${escapeHtml(payload.supervisor.name)}</h5><p>مشرف البلاغات</p></div><span>مُعيّن</span><dl><div><dt>الوردية</dt><dd>${escapeHtml(payload.supervisor.shift)}</dd></div><div><dt>القسم</dt><dd>${escapeHtml(payload.supervisor.section || payload.context.section)}</dd></div></dl></section>` : '<section class="engineering-followup__leader engineering-followup__leader--warning" role="status"><strong>لا يوجد مشرف بلاغات معيّن للوردية الحالية.</strong><p>يمكن تسجيل البلاغات، لكنها ستبقى بانتظار التعيين.</p></section>';
    body.innerHTML = `<section class="engineering-followup__context" aria-label="سياق الباب"><span>الباب: ${escapeHtml(payload.door.number)}</span><span>القسم: ${escapeHtml(payload.context.section)}</span><span>الحالة: ${escapeHtml(payload.context.status)}</span><strong>الجهة المالكة: ${escapeHtml(payload.context.owner)}</strong></section>${supervisor}<section class="engineering-followup__summary"><article><small>بلاغات نشطة</small><strong>${payload.summary.open}</strong></article><article><small>بلاغات اليوم</small><strong>${payload.summary.today}</strong></article><article><small>قيد المعالجة</small><strong>${payload.summary.processing}</strong></article><article><small>مصعّدة</small><strong>${payload.summary.escalated}</strong></article><article><small>مرتبطة بالصيانة</small><strong>${payload.summary.maintenance}</strong></article><article><small>مغلقة اليوم</small><strong>${payload.summary.closed_today}</strong></article></section><nav class="engineering-followup__filters" aria-label="تصفية بلاغات الباب"><button type="button" data-followup-filter-value="all" aria-pressed="true">الكل</button><button type="button" data-followup-filter-value="open">الحالة: نشطة</button><button type="button" data-followup-filter-value="high">الأولوية: عالية</button><button type="button" data-followup-filter-value="escalated">التصعيد</button><button type="button" data-followup-filter-value="maintenance">الصيانة</button><button type="button" data-followup-filter-value="closed">مغلقة</button></nav><div class="engineering-followup__list">${payload.incidents.length ? payload.incidents.map(incidentMarkup).join("") : `<div class="engineering-followup__empty"><span aria-hidden="true">✓</span><strong>لا توجد بلاغات على هذا الباب</strong><p>لا توجد بلاغات مسجلة ضمن النطاق الحالي.</p>${createAction}</div>`}</div><footer class="engineering-followup__footer"><a href="${escapeHtml(payload.incident_center_url)}">فتح في مركز البلاغات</a></footer>`;
